...
export default function Topbar({ userId, username, avatar }: { userId: string; username: string; avatar: string }) {
  return (
    <nav className="topbar">
      ...
      {username && avatar && (
        <Link
          className="flex items-center bg-zinc-700 hover:bg-zinc-600 duration-200 rounded-lg px-4 py-2 gap-2"
          href={`profile/${userId}`}
        >
          <Image src={avatar} alt="user-avatar" className="rounded-full" width={36} height={36} priority />
          <p className="text-base-regular text-zinc-300">{username}</p>
        </Link>
      )}
